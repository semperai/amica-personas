...
/**
 * @title UpgradeAmicaBridged
 * @notice Upgrade script for AMICA token on Arbitrum One and Base to V2
 * @dev Upgrades existing AmicaTokenBridged proxy to V2 implementation
 */
contract UpgradeAmicaBridged is Script {
    ...
    function run() external returns (UpgradeResult memory) {
        ...
        address proxyAddress = vm.envAddress("AMICA_BRIDGED_PROXY");

        string memory network = _getNetworkName();

        console2.log("========================================");
        console2.log("Upgrading AMICA Token to V2 (Bridged)");
        console2.log("========================================");
        ...
        AmicaTokenBridgedV2 upgraded = AmicaTokenBridgedV2(proxyAddress);
