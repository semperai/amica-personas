...
abstract contract DeployConfig is Script {
     ...
     */
    function getTokenAddress(uint256 chainId, string memory tokenSymbol)
        public
        pure
        returns (address)
    {
        if (chainId == 8453) {
            ...
            if (keccak256(bytes(tokenSymbol)) == keccak256(bytes("USDC"))) {
                ...
                keccak256(bytes(tokenSymbol)) == keccak256(bytes("WETH"))
            ) {
...
contract DeployAmicaWithConfig is DeployConfig {
    ...
    function run() public {
        ...
        require(
            config.positionManager != address(0),
            "PositionManager not configured for this chain"
        );
