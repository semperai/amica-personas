...
contract BondingCurveTest_Extended is Test {
    ...
    function test_222M_PriceProgression() public {
        ...
        for (uint256 i = 1; i < checkpoints.length; i++) {
            assertGt(checkpoints[i], checkpoints[i-1], "Price should increase monotonically");
        }
    ...
    function test_222M_ChunkedPurchases() public {
        ...
        for (uint256 i = 0; i < 10; i++) {
            uint256 cost = bondingCurve.calculateCostBetween(totalBought, totalBought + chunkSize, supply);
            uint256 tokensOut = bondingCurve.calculateAmountOut(cost, totalBought, supply);
            
            assertApproxEqRel(tokensOut, chunkSize, 0.001 ether, "Should buy expected chunk");
        ...
        // Total should match
        uint256 directTotal = bondingCurve.calculateCostBetween(0, supply, supply);
        assertApproxEqRel(totalSpent, directTotal, 0.001 ether, "Chunked cost equals direct");
    }
    ...
    function test_333M_BuySellSymmetry() public {
        ...
        for (uint256 i = 0; i < startPoints.length; i++) {
            ...
            uint256 tokensOut = bondingCurve.calculateAmountOut(buyAmount, startSold, supply);
            ...
            assertApproxEqRel(refund, buyAmount, 0.0001 ether, "Perfect symmetry required");
            console.log("333M - Symmetry test at", (startSold * 100) / supply, "% passed");
        }
    }
    ...
    function test_Compare_SupplyScaling() public {
        ...
        for (uint256 progress = 0; progress <= 100; progress += 10) {
            ...
            uint256 price222 = bondingCurve.getCurrentPrice(sold222, SUPPLY_222M);
            uint256 price333 = bondingCurve.getCurrentPrice(sold333, SUPPLY_333M);
            
            assertApproxEqRel(price222, price333, 0.001 ether, "Prices should match at same progress %");
    ...
    function test_Compare_CostPerToken() public {
        ...
        uint256 tokens222 = bondingCurve.calculateAmountOut(testAmount, start222, SUPPLY_222M);
        uint256 tokens333 = bondingCurve.calculateAmountOut(testAmount, start333, SUPPLY_333M);
        
        uint256 pricePerToken222 = (testAmount * PRECISION) / tokens222;
        uint256 pricePerToken333 = (testAmount * PRECISION) / tokens333;
        
        assertApproxEqRel(pricePerToken222, pricePerToken333, 0.001 ether, "Price per token should match");
    }
    ...
    function test_EdgeCase_BuyEntireSupply() public {
        ...
        for (uint256 i = 0; i < supplies.length; i++) {
            ...
            uint256 tokensOut = bondingCurve.calculateAmountOut(totalCost, 0, supply);
            assertApproxEqRel(tokensOut, supply, 0.001 ether, "Should buy entire supply");
            
            console.log("Supply", supply / 1e18 / 1e6, "M - Total cost:", totalCost / 1e18);
        }
    }
    ...
    function test_EdgeCase_MinimumPurchase() public {
        ...
        for (uint256 i = 0; i < supplies.length; i++) {
            ...
            uint256 tokensOut = bondingCurve.calculateAmountOut(reasonableMin, 0, supply);
            ...
            uint256 tokensOutEnd = bondingCurve.calculateAmountOut(reasonableMin, almostAllSold, supply);
            assertLt(tokensOutEnd, tokensOut, "Should receive fewer tokens at end");
    ...
    function test_EdgeCase_LargePurchaseNearEnd() public {
        ...
        uint256 tokensOut = bondingCurve.calculateAmountOut(largeAmount, almostSold, supply);
        assertLe(tokensOut, 1000 ether, "Cannot buy more than available");
        
        // Should get exactly remaining tokens
        uint256 costForRemaining = bondingCurve.calculateCostBetween(almostSold, supply, supply);
        uint256 exactTokensOut = bondingCurve.calculateAmountOut(costForRemaining, almostSold, supply);
        assertApproxEqRel(exactTokensOut, 1000 ether, 0.001 ether, "Should get exactly remaining");
    }
    ...
    function test_EdgeCase_SellEntirePosition() public {
        ...
        // Calculate refund
        uint256 refund = bondingCurve.calculateAmountOutForSellNoFee(position, position, supply);
        
        // Buy that position first to get cost
        uint256 cost = bondingCurve.calculateCostBetween(0, position, supply);
        
        assertApproxEqRel(refund, cost, 0.0001 ether, "Should refund exact cost");
    }
    ...
    function test_FeeImpact_RoundTrip() public {
        ...
        for (uint256 i = 0; i < supplies.length; i++) {
            ...
            for (uint256 j = 0; j < amounts.length; j++) {
                ...
                // Buy tokens
                uint256 tokensOut = bondingCurve.calculateAmountOut(buyAmount, 0, supply);
                
                // Sell back immediately
                uint256 refundWithFee = bondingCurve.calculateAmountOutForSell(tokensOut, tokensOut, supply);
                uint256 refundNoFee = bondingCurve.calculateAmountOutForSellNoFee(tokensOut, tokensOut, supply);
                ...
                assertApproxEqAbs(actualFee, expectedFee, 1, "Fee should be exactly 0.1%");
                
                // Loss should be at least 10 bps (0.1%), can be exactly 10 for large amounts
                assertGe(lossPercent, 10, "Loss should be >= 10 bps (0.1%)");
                assertLt(lossPercent, 15, "Loss should be < 15 bps for reasonable amounts");
            }
        }
    }
    ...
    function test_GasBenchmark_Operations() public {
        ...
        for (uint256 i = 0; i < supplies.length; i++) {
            ...
            gasAfter = gasleft();
            console.log("  calculateAmountOutForSell gas:", gasBefore - gasAfter);
            ...
            // Cost between
            gasBefore = gasleft();
            bondingCurve.calculateCostBetween(midPoint, midPoint + 1000 ether, supply);
            gasAfter = gasleft();
    ...
    function test_VirtualReserves_Consistency() public {
        ...
        for (uint256 i = 0; i < supplies.length; i++) {
            ...
            (uint256 vToken0, uint256 vETH0) = bondingCurve.getVirtualReserves(0, supply);
            ...
            for (uint256 progress = 10; progress <= 100; progress += 10) {
                ...
                (uint256 vToken, uint256 vETH) = bondingCurve.getVirtualReserves(sold, supply);
                uint256 currentK = vToken * vETH;
                
                // Allow tiny rounding differences (0.000001%)
                assertApproxEqRel(currentK, k, 0.000001 ether, "k should remain approximately constant");
                
                // Verify price calculation
                uint256 price = bondingCurve.getCurrentPrice(sold, supply);
                uint256 expectedPrice = (vETH * PRECISION) / vToken;
                assertEq(price, expectedPrice, "Price should match virtual reserve ratio");
            }
        }
    }
    ...
    function testFuzz_BuySellSymmetry_222M(uint256 buyAmount, uint256 initialSold) public {
    ...
    function testFuzz_BuySellSymmetry_333M(uint256 buyAmount, uint256 initialSold) public {
        buyAmount = bound(buyAmount, 1 ether, 50000 ether);
    ...
    function _testBuySellSymmetryForSupply(
        ...
        // Buy tokens
        uint256 tokensBought = bondingCurve.calculateAmountOut(buyAmount, initialSold, supply);
    ...
    function testFuzz_MonotonicPriceIncrease(uint256 sold1, uint256 sold2) public {
        uint256 supply = SUPPLY_333M;
        ...
        if (sold2 < earlyThreshold || sold1 > lateThreshold) {
            ...
            assertGt(price2, price1, "Price must increase for meaningful gaps in active range");
        }
    }
    ...
    function testFuzz_CostBetweenConsistency(
        ...
        uint256 costTotal = bondingCurve.calculateCostBetween(start, end, supply);
        
        assertApproxEqRel(cost1 + cost2, costTotal, 0.0001 ether, "Costs should be additive");
    }
}
