...
contract BondingCurveTest_Security is Test {
    ...
    function test_Overflow_LargeAmounts() public {
        ...
        // Should not overflow
        uint256 tokensOut = bondingCurve.calculateAmountOut(largeAmount, 0, supply);
    ...
    function test_Underflow_SmallAmounts() public {
        ...
        uint256 tokensOutEnd = bondingCurve.calculateAmountOut(reasonableMin, nearEnd, supply);
    ...
    function test_PrecisionLoss_RoundingErrors() public {
        ...
        for (uint256 i = 0; i < 1000; i++) {
            ...
            uint256 tokensOut = bondingCurve.calculateAmountOut(smallAmount, currentSold, supply);
        ...
        uint256 singleTradeOut = bondingCurve.calculateAmountOut(totalIn, 0, supply);
        
        // Should be very close (within 0.01%)
        assertApproxEqRel(totalOut, singleTradeOut, 0.0001 ether, "Minimal precision loss");
    ...
    function test_ExploitPrevention_SandwichAttack() public {
        ...
        uint256 attackerTokens = bondingCurve.calculateAmountOut(attackerCapital, 0, supply);
        ...
        assertLt(attackerPnL, 0, "Sandwich attack should be unprofitable due to fee");
    }
    ...
    function test_ExploitPrevention_RoundingExploit() public {
        ...
        for (uint256 i = 0; i < exploitRounds; i++) {
            ...
            uint256 tokensOut = bondingCurve.calculateAmountOut(buyAmount, currentSold, supply);
        ...
        assertEq(totalProfit, 0, "No profit possible from rounding exploitation");
    }
    ...
    function test_Boundary_ExactSupplyPurchase() public {
        ...
        for (uint256 i = 0; i < supplies.length; i++) {
            ...
            assertGe(tokensOut, supply - 1 ether, "Should buy almost entire supply");
    ...
    function test_StateConsistency_VirtualReserves() public {
        ...
        for (uint256 i = 1; i <= 10; i++) {
            ...
            (uint256 x, uint256 y) = bondingCurve.getVirtualReserves(sold, supply);
            ...
            assertApproxEqRel(currentK, k, 0.000001 ether, "k must remain approximately constant");
        }
    }
    ...
    function test_MathProperties_Convexity() public {
        ...
        for (uint256 i = 0; i < positions.length; i++) {
            uint256 tokensOut = bondingCurve.calculateAmountOut(amount, positions[i], supply);
            uint256 tokensPerEth = (tokensOut * PRECISION) / amount;
            
            assertLt(tokensPerEth, lastTokensPerEth, "Tokens per ETH should decrease");
            lastTokensPerEth = tokensPerEth;
            
            console.log("Position", (positions[i] * 100) / supply, "% - Tokens per ETH:", tokensPerEth / 1e18);
        }
    }
    ...
    function test_Integration_CalculateCostBetween() public {
        ...
        for (uint256 start = 0; start < supply; start += supply / 10) {
            ...
            uint256 tokensBought = bondingCurve.calculateAmountOut(cost, start, supply);
            
            assertApproxEqRel(tokensBought, tokensTarget, 0.001 ether, "Cost calculation consistency");
        }
    }
    ...
    function test_Comparative_222M_vs_333M_Economics() public {
        ...
        for (uint256 i = 0; i < supplies.length; i++) {
            ...
            uint256 cost25 = bondingCurve.calculateCostBetween(0, supply / 4, supply);
            uint256 cost50 = bondingCurve.calculateCostBetween(0, supply / 2, supply);
            uint256 cost75 = bondingCurve.calculateCostBetween(0, (supply * 3) / 4, supply);
